...
PERMISSIONS = {
    'admin':  {'view', 'search', 'add', 'edit', 'delete', 'hard_delete', 'export',
               'settings', 'users', 'mukhtars', 'regions', 'backup', 'families'},
    'editor': {'view', 'search', 'add', 'edit', 'export', 'mukhtars', 'regions', 'families'},
    'viewer': {'view', 'search'},
}
...
def update_person(pid: int, الاسم='', التولد='', المهنة='', القضاء='',
                  ...
                  الحالة_الزوجية='', الصلة='', الملاحظات='') -> bool:
        ...
        conn.execute(
            """UPDATE persons
               SET الاسم=?, clean_name=?, التولد=?, المهنة=?, القضاء=?,
                   ...
                   الحالة_الزوجية=?, الصلة=?, الملاحظات=?,
            ...
            (الاسم.strip(), _cn(الاسم), التولد.strip(), المهنة.strip(), القضاء.strip(),
             ...
             الحالة_الزوجية, الصلة, الملاحظات.strip() if الملاحظات else '', pid),
        )
        return conn.total_changes > 0
...
def add_person(الاسم='', التولد='', المهنة='', القضاء='', العنوان='',
               ...
               المنطقة='', الحي='', mukhtar_id=None,
               الحالة_الزوجية='', الصلة='', family_id=None, الملاحظات='') -> int:
    mn = normalize_region(المنطقة) if المنطقة else (normalize_region(القضاء) if القضاء else '')
    with _conn() as conn:
        cur = conn.execute(
            """INSERT INTO persons
               (الاسم, clean_name, التولد, المهنة, القضاء, العنوان, المحل, الهاتف,
                الشيت, الصف, المنطقة, الحي, mukhtar_id, الحالة_الزوجية, الصلة, family_id, الملاحظات)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (الاسم.strip(), _cn(الاسم), التولد.strip(), المهنة.strip(), القضاء.strip(),
             العنوان.strip(), المحل.strip(), الهاتف.strip(),
             الشيت, الصف, mn, الحي.strip() if الحي else '', mukhtar_id,
             الحالة_الزوجية, الصلة, family_id, الملاحظات.strip() if الملاحظات else ''),
        )
        return cur.lastrowid
...
def update_neighborhood(nid: int, الاسم: str) -> bool:
    with _conn() as conn:
        conn.execute("UPDATE neighborhoods SET الاسم=? WHERE id=?", (الاسم.strip(), nid))
        return conn.total_changes > 0


def update_region_name(region_id: int, new_name: str) -> bool:
    with _conn() as conn:
        old = conn.execute("SELECT name FROM regions WHERE id=?", (region_id,)).fetchone()
        if not old:
            return False
        old_name = old[0]
        conn.execute("UPDATE regions SET name=? WHERE id=?", (new_name.strip(), region_id))
        conn.execute("UPDATE neighborhoods SET المنطقة=? WHERE المنطقة=?", (new_name.strip(), old_name))
        conn.execute("UPDATE persons SET المنطقة=? WHERE المنطقة=?", (new_name.strip(), old_name))
        return True
...
def create_family(head_pid: int) -> int:
    """ينشئ عائلة جديدة برقم فريد عشوائي (6 أرقام)"""
    import random
    with _conn() as conn:
        existing_ids = {r[0] for r in conn.execute(
            "SELECT DISTINCT family_id FROM persons WHERE family_id IS NOT NULL"
        ).fetchall()}
        while True:
            new_fid = random.randint(100000, 999999)
            if new_fid not in existing_ids:
                break
        conn.execute(
